...
実験ログを残す場合は `--trace-out chat_trace.jsonl` を付けると、モデル出力・tool observation・final message がJSONLで追えます。
実モデルでのmemory writeを即保存したくない場合は `--queue-writes` を付け、`review list/apply/reject` で確認できます。
...
Use `--trace-out chat_trace.jsonl` to append model outputs, tool observations, and final messages as JSONL experiment traces.
Use `--queue-writes` to review model-authored memory writes before applying them, and `--json-repair-steps N` to retry malformed JSON outputs.
